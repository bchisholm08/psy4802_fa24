...
```{r}
library(readr)
aggressionData<- read.csv("~/Desktop/FA24 Syllabi:Schedules/PSY 4802 Using R to Create Reproducible Research/Notes/PSY4802-AggressionSampleData.csv")
...
```{r}
religDat <- data(relig_income)
```

2. Restructure the dataset so that it is in long format. 
```{r}
# identifier is religion 
# predictor is levels of income 
# dependent var is frequency 
longReligion <-  relig_income %>%
  pivot_longer(c("<$10k","$10-20k","$20-30k","$30-40k","$40-50k","$50-75k","$75-100k","$100-150k",">150k","Don't know/refused"),names_to = "income",values_to = 'freq')

# much cooler and more efficient way to do this. 
# can use c() to get a list to input into the function, will not allow stacked conditionals. ! + || is not good code 
longReligion2 <- relig_income %>%
...
```{r}
library(tidyr)
# key = column we want old col names in
# value = values in cols 
aggressionData2 <- gather(aggressionData, key = "timepoint", value = "aggressionScore", c("baselineAggression","postAggression"))
```
...
```{r}
aggressionLong2 <- melt(aggressionData, id.vars = c("subjNo","ageGroup","parentReportGender","condition"), measure.vars = c("baselineAggression","postAggression"),variable.name = "timepoint",value.name = "aggressionScore")
```
### Practice wide to long with the billboard data set 
  What is the repeated measure in the data set? 
```{r}
data(billboard)
```
...
```{r}
agg_wide<- pivot_wider(aggressionLong, id_cols = c(colnames(aggressionLong)[1:4]), names_from = "timePoint", values_from = "aggressionScore")
# works well with pipes as well
```
